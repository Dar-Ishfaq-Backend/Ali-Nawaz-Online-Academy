...
import {
  ...
  Users, BarChart3, PlusCircle, Shield, X, Flame
} from 'lucide-react';
...
export default function Sidebar({ open, onClose }) {
  ...
  const superAdminNav = [
    ...
    { to: '/teacher', icon: PlusCircle, label: 'Content' },
    { to: '/admin/users', icon: Users, label: 'Users' },
    { to: '/admin/settings', icon: Shield, label: 'Super Controls' },
  ];
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full pt-4 pb-6">
      ...
      <div className="mx-3 px-3 py-2 rounded-lg"
        ...
        <div className="text-[10px] font-cinzel text-emerald-500/50 tracking-widest mb-0.5">ROLE</div>
        <div className="text-sm font-crimson text-emerald-400">{role}</div>
      </div>
    </div>
  );
